...
scoreboard players set #succeed ancValue 0
execute if predicate anc:machines/interact_machine/heavy_weighted_pressure_plate as @e[type=falling_block,predicate=anc:anvil/notfull,limit=1,distance=..10,sort=nearest,tag=!ancRepairTrue] run function anc:machines/interact_machine/interact/repair/anvil
