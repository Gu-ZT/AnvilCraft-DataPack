...
execute if score $ancAttractingFalling ancConfig matches 1 if score #check ancValue matches 1 as @e[type=falling_block,predicate=anc:anvil,dx=0,dz=0,dy=-4,sort=nearest,limit=1] unless data entity @s {NoGravity:1b} positioned ~.5 ~ ~.5 run function anc:machines/magnet_block/keep
