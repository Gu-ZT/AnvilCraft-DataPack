...
execute if score $ancTemp ancItemCount matches 1.. as @e[type=minecraft:item,predicate=anc:recipes/puffing,distance=..1] at @s if block ~ ~ ~ #minecraft:cauldrons store success score $puffing ancSuccess run function anc:recipes/puffing/puffing
