...
execute if score #exe_count ancValue matches 1.. align xyz positioned ~.5 ~.5 ~.5 if block ~ ~ ~ #minecraft:cauldrons if entity @e[type=minecraft:item,nbt={Item:{tag:{id:"anc:tear_of_the_sea"}}},distance=..1] run function anc:recipes/compressing/tear_of_the_sea_loot
